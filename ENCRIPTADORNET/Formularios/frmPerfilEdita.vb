...
ActualizaPerfil:
    ...
    Private Sub cmdEliminar_Click(sender As Object, e As EventArgs) Handles cmdEliminar.Click
        ...
        gs_Sql = gs_Sql & " AND PU.usuario = US.usuario and US.password not in ('ANULADO', 'BLOQUEAR')" 'cpb 25-feb-04
        'dbExecQuery gs_Sql
        'dbGetRecord
        dtRespuesta = objDataSourse.RealizaConsulta(gs_Sql)
        'If dbError = 0 Then
...
EliminaPerfil:
    ...
    Private Sub cmdAgregar_Click(sender As Object, e As EventArgs) Handles cmdAgregar.Click
        ...
        Else
            ...
            gs_Sql = gs_Sql & "WHERE nombre_perfil = '" & UCase(Trim(txtNombrePerfil.Text)) & "'"
            'dbExecQuery gs_Sql
            'dbGetRecord
            dtRespuesta = objDataSourse.RealizaConsulta(gs_Sql)
            'If dbError = 0 Then
...
RegistraPerfil:
            ...
            gs_Sql = gs_Sql & "(" & 1 & ", '" & TotalAccesos() & "', '" & TotalAutorizaciones() & "', '" & UCase(Trim(txtNombrePerfil.Text)) & "', getdate())" '---gs_Sql = gs_Sql & "(" & (NumAplicacion.Replace("(", "")).Replace(")", "") & ", '" & TotalAccesos() & "', '" & TotalAutorizaciones() & "', '" & UCase(Trim(txtNombrePerfil.Text)) & "', getdate())"
            'dbExecQuery gs_Sql
            NumRegistros = objDatasource.insertar(gs_Sql)
            'If dbError <> 0 Then
            If NumRegistros <= 0 Then
