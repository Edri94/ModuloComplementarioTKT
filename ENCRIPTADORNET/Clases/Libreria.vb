...
#Region "Permisos Seguridad"
    ...
    Public Sub CargaPermisos()
        ...
        gn_TotalPermisos = 0
        ls_sql = "SELECT count(*) FROM CATALOGOS.dbo.PERMISOS_HEXA WHERE aplicacion in (1,2) "
        ...
        If gn_TotalPermisos > 0 Then
            ...
            ls_sql &= " aplicacion in (1,2)  order by aplicacion,descripcion" '-----RACB 09/08/2021
            ' LEO valor_permiso b
            i = 0

            dtPermisos = d.Consulta(ls_sql, "Permisos")

            Do While Not (i = (gn_TotalPermisos - 1))
    ...
    '2020 junio 09
    '-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=---
    Public Sub CargaAutorizaciones()
        Dim d As New Datasource
        ...
        ls_sql = "SELECT count(*) FROM CATALOGOS.dbo.AUTORIZACIONES_HEXA WHERE aplicacion in (1,2)"
        ...
        If gn_TotalAutorizaciones > 0 Then
            ...
            ls_sql &= "aplicacion in (1,2) order by aplicacion,descripcion" 'ls_sql &= "aplicacion = 1 order by descripcion" '------------- RACB 09/08/2021
